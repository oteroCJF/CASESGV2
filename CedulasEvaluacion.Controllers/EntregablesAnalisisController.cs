...
namespace CedulasEvaluacion.Controllers
{
    public class EntregablesAnalisisController : Controller
    {
        ...
        public EntregablesAnalisisController(IRepositorioEntregablesCedula evAnalisis, IRepositorioEvaluacionServicios iCedula, IHostingEnvironment environment)
        {
        ...
        [HttpGet]
        [Route("/analisis/getEntregables/{id?}/{servicio?}")]
        public async Task<IActionResult> getEntregablesAnalisis(int id,int servicio)
        {
            List<Entregables> entregables = null;
            CedulaEvaluacion cedula = await vCedula.CedulaById(id);
            entregables = await eAnalisis.getEntregables(id, servicio);
            string table = "";
            string tipo = "";
            if (entregables != null)
            {
        ...
        [HttpGet]
        [Route("/analisis/getListadoEntregables/{id?}/{servicio?}")]
        public async Task<IActionResult> getListadoEntregablesAnalisis(int id,int servicio)
        {
            List<Entregables> entregables = null;
            entregables = await eAnalisis.getEntregables(id,servicio);
            if (entregables != null)
        ...
        {
            int success = 0;
            success = await eAnalisis.eliminaEntregables(entregable);
            if (success != -1)
            {
                return Ok();
            }
