...
{
    public class EntregablesComedorController : Controller
    {
        private readonly IRepositorioEntregablesCedula eComedor;
        private readonly IRepositorioEvaluacionServicios vCedula;
        private readonly IHostingEnvironment environment;

        public EntregablesComedorController(IRepositorioEntregablesCedula evComedor, IRepositorioEvaluacionServicios iCedula, IHostingEnvironment environment)
        {
            this.vCedula = iCedula ?? throw new ArgumentNullException(nameof(iCedula));
            this.eComedor = evComedor ?? throw new ArgumentNullException(nameof(evComedor));
            this.environment = environment;
        }

        /*Metodo para adjuntar los entregables*/
        [HttpPost]
        [Route("/comedor/adjuntaEntregable")]
        public async Task<IActionResult> adjuntaEntregable([FromForm] Entregables entregables)
        {
            int success = 0;
            success = await eComedor.adjuntaEntregable(entregables);
            if (success != 0)
            {
                return Ok();
            }
        ...
        [HttpGet]
        [Route("/comedor/getEntregables/{id?}/{servicio}")]
        public async Task<IActionResult> getEntregablesComedor(int id, int servicio)
        {
            List<Entregables> entregables = await eComedor.getEntregables(id, servicio);
        ...
        [HttpGet]
        [Route("/comedor/verArchivo/{folio?}/{nombre?}")]
        ...
        [HttpPost]
        [Route("/comedor/eliminaArchivo")]
        public async Task<IActionResult> eliminaArchivo([FromBody] Entregables entregable)
        {
            int success = 0;
            success = await eComedor.eliminaEntregables(entregable);
            if (success != -1)
            {
                return Ok();
            }
        ...
        [HttpGet]
        [Route("/comedor/buscaEntregable/{id?}/{tipo?}")]
        public async Task<IActionResult> buscaEntregable(int id, string tipo)
        {
            int exists = 0;
            exists = await eComedor.buscaEntregable(id, tipo);
            if (exists != -1)
            {
                return Ok(exists);
            }
        ...
        [HttpPost]
        [Route("/comedor/Entregables/autoRecha")]
        public async Task<IActionResult> aprovacionRechazoCedula([FromBody] Entregables entregables)
        {
            int success = await eComedor.apruebaRechazaEntregable(entregables);
            if (success != 0)
            {
                return Ok();
            }
        ...
        [HttpPost]
        [Route("/comedor/entregables/historialEntregable")]
        public async Task<IActionResult> historialEntregable([FromBody] HistorialEntregables historialEntregables)
        {
            int success = 0;
            success = await eComedor.capturaHistorial(historialEntregables);
            if (success != 0)
            {
                return Ok();
            }
